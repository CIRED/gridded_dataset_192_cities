...
cityDatabaseDf = (cityDatabaseDf >> dp.select(dp.X.City, dp.X.Country, dp.X.Continent, dp.X.GridEPSG, dp.X.TransportSource, dp.X.UTC_Jan) >> 
...
cityDatabaseDf = cityDatabaseDf[147:len(cityDatabaseDf)];
cityDatabaseDf = cityDatabaseDf.reset_index();

# Rush hours (Thursdays )
rushHoursList = {'rushHour' : [datetime.datetime(2020, 2, 6, 16, 0, 0), \
                               datetime.datetime(2020, 2, 6, 16, 30, 0), \
                               datetime.datetime(2020, 2, 6, 17, 0, 0), \
                               datetime.datetime(2020, 2, 6, 17, 30, 0), \
                               datetime.datetime(2020, 2, 6, 18, 0, 0), \
                               datetime.datetime(2020, 2, 6, 18, 30, 0), \
                               datetime.datetime(2020, 2, 6, 19, 0, 0), \
                               datetime.datetime(2020, 2, 6, 19, 30, 0), \
                               datetime.datetime(2020, 2, 6, 20, 0, 0)]};
                               
rushHoursDf = pandas.DataFrame(data = rushHoursList);
...
for i in range(0, len(cityDatabaseDf)):
    ...
    gridName = 'googleGrid_' + cityName + '_' + gridEPSG + '_RANDOMV1' + '.csv';
    ...
    cityRushHoursDf = rushHoursDf;
    cityRushHoursDf = (cityRushHoursDf >> dp.mutate(UTC_RushHour = dp.X.rushHour - datetime.timedelta(hours = cityDatabaseDf.UTC_Jan[i])) >>
                                          dp.mutate(rushHourTimestamp = (dp.X.UTC_RushHour - datetime.datetime(1970, 1, 1))))
                                                                     
    
    ######################
    # Loop over rush hours
    for timeIndex in range(0, len(cityRushHoursDf)):
        
        # Change directory    
        os.chdir(path_GoogleAPI);
    
        # Time of evaluation
        transportTime = str(cityRushHoursDf.rushHour[timeIndex].hour) + 'h' + str(cityRushHoursDf.rushHour[timeIndex].minute);
        ...
        transportTimestamp = int(cityRushHoursDf.rushHourTimestamp[timeIndex].total_seconds());
        transportTimeDate = datetime.datetime.isoformat(cityRushHoursDf.rushHour[timeIndex], '-');
