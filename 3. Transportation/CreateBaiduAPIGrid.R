...
cityDatabaseDf = cityDatabaseDf %>% filter(PopDensityYear == '2015') %>%
  filter(Country == 'China' & TransportYear != '2018') %>%
...
CreateBaiduAPIGrid = function(cityDataDf, nbPoints, nbLines, dZero, thetaZero)
{
  # Import grid
  ...
  # Creating grid
  baiduGridDf = data.frame(x = double(),
                            y = double(),
                            lineID = integer(),
                            theta = double(),
                            dMax = double())
  ...
  for(i in (1:nbLines))
  {
    ...
    baiduGridDfTmp = data.frame(matrix(ncol = 5, nrow = length(x)));
    colnames(baiduGridDfTmp) = c('x', 'y', 'lineID', 'theta', 'dMax');
    
    baiduGridDfTmp$x = x;
    baiduGridDfTmp$y = y;
    baiduGridDfTmp$lineID = i;
    baiduGridDfTmp$theta = linesDf$theta[i];
    baiduGridDfTmp$dMax = linesDf$d[i];
    
    baiduGridDf = rbind(baiduGridDf, baiduGridDfTmp);
    
    rm(baiduGridDfTmp);
  }
  
  baiduGridSf = baiduGridDf %>% st_as_sf(coords = c('x', 'y'),
                                           crs = st_crs(gridSf))
  
  baiduGridSf = cbind(baiduGridSf, st_coordinates(baiduGridSf));
  
  # Convert to WGS84
  baiduGridSf = st_transform(baiduGridSf, crs = 4326)
  baiduGridSf = cbind(baiduGridSf, st_coordinates(baiduGridSf));
  
  names(baiduGridSf)[names(baiduGridSf) == 'X.1'] <- 'wgsLong';
  names(baiduGridSf)[names(baiduGridSf) == 'Y.1'] <- 'wgsLat';
...
CreateBaiduAPIGrid_OceanIncluded = function(cityDataDf, nbPoints, nbLines, dZero, thetaZero)
{
  # Import grid
  gridSf = st_read(dsn = paste('./Data/', cityDataDf$Country, '/', cityDataDf$City, '/Grid/grille_', toupper(cityDataDf$City), '_finale_', cityDataDf$GridEPSG, '.shp',
                               sep = ''))
...
CreateRandomBaiduAPIGrid = function(cityDataDf, nbPoints)
{
  # Import grid
  gridSf = st_read(dsn = paste('./Data/', cityDataDf$Country, '/', cityDataDf$City, '/Grid/grille_', toupper(cityDataDf$City), '_finale_', cityDataDf$GridEPSG, '.shp',
                               sep = ''))
  ...
  # Creating grid
  baiduGridDf = data.frame(x = double(nbPoints),
                            y = double(nbPoints));
  ...
  baiduGridDf$x = runif(nbPoints, xmin, xmax);
  # Get random y inside bbox
  baiduGridDf$y = runif(nbPoints, ymin, ymax);
...
CreateRandomBaiduAPIGrid_OceanIncluded = function(cityDataDf, nbPoints)
{
  # Import grid
  gridSf = st_read(dsn = paste('./Data/', cityDataDf$Country, '/', cityDataDf$City, '/Grid/grille_', toupper(cityDataDf$City), '_finale_', cityDataDf$GridEPSG, '.shp',
                               sep = ''))
  ...
  baiduGridSf = st_sample(bboxSf, size = nbPoints, type = "random");
  
  baiduGridDf = as.data.frame(st_coordinates(baiduGridSf))
  baiduGridSf = baiduGridDf %>% st_as_sf(coords = c('X', 'Y'),
                                           crs = st_crs(gridSf))
  baiduGridSf = cbind(baiduGridSf, st_coordinates(baiduGridSf));
...
dZero = 400;
thetaZero = pi/8;
gridNameDetail = 'RushHour_'


# Initialize GUI progress bar
progressBar = tkProgressBar(title = 'Creating grid for Baidu API...',
                            min = 0,
...
# 
#   baiduGridSf = CreateBaiduAPIGrid(cityDataDf, nbPoints, nbLines, dZero, thetaZero);
# 
#   # Save result
#   ifelse(!dir.exists(file.path(paste('/home/quentin/Documents/Baidu_API_Transports/Data/', cityDataDf$City, '/Results', sep = ''))),
#          dir.create(file.path(paste('/home/quentin/Documents/Baidu_API_Transports/Data/', cityDataDf$City, '/Results', sep = ''))), FALSE)
#   
#   path = file.path(paste('/home/quentin/Documents/Baidu_API_Program/Data/', cityDataDf$City, '/Destinations/', sep = ''));
#   fileName = paste('Destinations_Grid_', cityDataDf$City, '_', cityDataDf$GridEPSG,  '.csv', sep = '');
#   
#   # Save Baidu grid
#   st_write(baiduGridSf, paste(path, '/', fileName, sep = ''),
#            delete_dsn = TRUE)
#   
#   # Save WGS84 Center
#   SaveWGS84Center(cityDataDf = cityDataDf,
#                   path = gsub('Destinations', 'Origin', path),
#                   fileName = paste('Origin_', cityDataDf$City, '_4326.csv', sep = ''));
# }
...
for(i in (1:length(cityDatabaseDf$City)))
{
  ...
  baiduGridSf = CreateBaiduAPIGrid_OceanIncluded(cityDataDf, nbPoints, nbLines, dZero, thetaZero);
...
#   setTkProgressBar(progressBar, barCount, label = paste(round((barCount/nrow(cityDatabaseDf)) * 100, 1), '% completed [', cityDataDf$City, ']', sep = ''));
# 
#   baiduGridSf = CreateRandomBaiduAPIGrid(cityDataDf, nbPoints);
# 
#   # Save result
#   ifelse(!dir.exists(file.path(paste('/home/quentin/Documents/Baidu_API_Transports/Data/', cityDataDf$City, '/Results', sep = ''))),
#          dir.create(file.path(paste('/home/quentin/Documents/Baidu_API_Transports/Data/', cityDataDf$City, '/Results', sep = ''))), FALSE)
...
#   setTkProgressBar(progressBar, barCount, label = paste(round((barCount/nrow(cityDatabaseDf)) * 100, 1), '% completed [', cityDataDf$City, ']', sep = ''));
# 
#   baiduGridSf = CreateRandomBaiduAPIGrid_OceanIncluded(cityDataDf, nbPoints);
# 
#   # Save result
#   ifelse(!dir.exists(file.path(paste('/home/quentin/Documents/Baidu_API_Transports/Data/', cityDataDf$City, '/Results', sep = ''))),
#          dir.create(file.path(paste('/home/quentin/Documents/Baidu_API_Transports/Data/', cityDataDf$City, '/Results', sep = ''))), FALSE)
